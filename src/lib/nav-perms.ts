...
import { NAV_SECTIONS, personaSees, moduleAllowed, type NavSection, type WorkshopPersona } from "@/lib/nav-registry";

/**
 * 角色导航（DB Permission 覆盖感知）：Permission 行优先于默认矩阵。
 * Developer Settings 关闭某角色×模块 → 导航即时隐藏。
 */
export async function navForRoleWithPerms(orgId: string, role: string, persona: WorkshopPersona): Promise<NavSection[]> {
