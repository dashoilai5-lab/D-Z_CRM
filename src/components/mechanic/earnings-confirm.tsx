...
export function EarningsConfirm({ payouts }: { payouts: PendingPayout[] }) {
  ...
  const approve = (id: string) =>
    start(async () => {
      ...
      if (r.ok) { toast.success("Approved — workshop will confirm payment"); router.refresh(); }
      ...
      {payouts.map((p) => (
        <div key={p.id} className="rounded-2xl border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold text-amber-700 dark:bg-amber-950/60 dark:text-amber-300">AWAITING APPROVAL</span>
          </div>
          <button
            type="button"
          ...
          >
            <BadgeCheck className="h-4 w-4" /> Approve payment · {formatRM(p.totalSen)}
          </button>
        </div>
      ))}
    </>
