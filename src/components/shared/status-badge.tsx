...
const JOB_STYLES: Record<string, { key: string; cls: string }> = {
  WAITING: { key: "status.WAITING", cls: "bg-slate-100 text-slate-700 dark:bg-slate-950/60 dark:text-slate-300 border-slate-200" + DARK },
...
const REMINDER_STYLES: Record<string, { key: string; cls: string }> = {
  UPCOMING: { key: "rem.UPCOMING", cls: "bg-slate-100 text-slate-700 dark:bg-slate-950/60 dark:text-slate-300 border-slate-200" + DARK },
...
export function StatusBadge({ kind, value }: { kind: "job" | "booking" | "reminder"; value: string }) {
  ...
  const s = map[value] ?? { key: "common.status", cls: "bg-slate-100 text-slate-700 dark:bg-slate-950/60 dark:text-slate-300 border-slate-200" };
