...
export function DemoBar({ persona, compact = false }: { persona: DemoPersona; compact?: boolean }) {
  ...
  return (
    <div className="sticky top-0 z-40 border-b bg-amber-50/90 backdrop-blur print:hidden">
      <div className={`mx-auto flex items-center gap-2 px-3 md:px-6 ${compact ? "h-10" : "h-11"}`}>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/15 px-2.5 py-0.5 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-500/30">
        ...
        >
          <SelectTrigger className="h-7 w-[190px] text-xs bg-white" disabled={pending}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {DEMO_PERSONAS.map((p) => (
