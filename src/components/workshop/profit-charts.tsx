  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <AreaChart data={rows} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
        ...
        <Area type="monotone" dataKey="revenue" name="Revenue" stroke="oklch(0.62 0.19 45)" strokeWidth={2} fill="url(#rev)" />
        <Area type="monotone" dataKey="grossProfit" name="Gross Profit" stroke="#10b981" strokeWidth={2} fill="transparent" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
...
export function RevenueSplitChart({ service, parts }: { service: number; parts: number }) {
  const data = [
    { name: "Service Revenue", value: Math.round(service / 100) },
    { name: "Parts Revenue", value: Math.round(parts / 100) },
  ];
  return (
