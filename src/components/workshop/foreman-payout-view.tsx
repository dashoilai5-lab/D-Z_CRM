...
export function ForemanPayoutView({ foremen, lang }: { foremen: ForemanBill[]; lang: Lang }) {
  ...
  const doAgree = () =>
    start(async () => {
      if (!agreeFor) return;
      const r = await agreePayout(agreeFor.payoutId, agreeMethod);
      if (r.ok) { toast.success("Payment confirmed — salary paid"); setAgreeFor(null); router.refresh(); }
      else toast.error(r.error);
    });
  ...
  return (
    <>
                    ...
                    return (
                      <div key={k} className={paid ? "opacity-60" : ""}>
                        ...
                        <div className="flex items-center gap-3 px-4 py-2.5">
                          ...
                          <span className={"shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold " + (paid ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300" : b.payoutStatus === "MECHANIC_APPROVED" ? "bg-blue-100 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300" : partial ? "bg-amber-100 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300")}>
                            {paid ? t("payout.paid", lang) : b.payoutStatus === "MECHANIC_APPROVED" ? "MECHANIC OK" : partial ? t("payout.partial", lang) : t("payout.unpaid", lang)}
                          </span>
                          {!paid && b.payoutStatus === "MECHANIC_APPROVED" && b.payoutId && (
                            <Button size="sm" className="shrink-0" onClick={() => setAgreeFor({ payoutId: b.payoutId!, name: f.name, totalSen: b.totalSen })}>
                              Agree & pay
                            </Button>
                          )}
      ...
      {/* 最终出粮确认（mechanic 已 approve，workshop agree） */}
      <Dialog open={!!agreeFor} onOpenChange={(o) => !o && setAgreeFor(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Agree & pay salary · {agreeFor?.name}</DialogTitle>
            <DialogDescription>Mechanic has approved — confirm to release {agreeFor ? formatRM(agreeFor.totalSen) : ""}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div>
              <Label>Method</Label>
              <div className="mt-1.5 grid grid-cols-2 gap-2">
                {["CASH", "QR"].map((m) => (
                  <button key={m} type="button" onClick={() => setAgreeMethod(m)} className={"rounded-xl border px-3 py-2.5 text-sm font-semibold transition-colors " + (agreeMethod === m ? "border-primary bg-primary text-primary-foreground" : "bg-card hover:bg-accent")}>
                    {m === "CASH" ? "💵 Cash" : "📱 QR"}
                  </button>
                ))}
              </div>
