...
export function SlotManager({ branches }: { branches: { id: string; label: string }[] }) {
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      <h2 className="font-semibold text-sm mb-3">Generate slots</h2>
      <div className="grid sm:grid-cols-5 gap-3 items-end">
        <div>
          <label className="text-[11px] font-medium text-muted-foreground block mb-0.5">Branch</label>
        ...
        <div>
          <label className="text-[11px] font-medium text-muted-foreground block mb-0.5">Days ahead</label>
        ...
        <div className="sm:col-span-2">
          <label className="text-[11px] font-medium text-muted-foreground block mb-0.5">Times (comma-separated)</label>
          <input className="w-full rounded-md border bg-background px-3 py-2 text-sm" value={times} onChange={(e) => setTimes(e.target.value)} />
        </div>
        <div>
          <label className="text-[11px] font-medium text-muted-foreground block mb-0.5">Max per slot</label>
      ...
      <button
        ...
        onClick={async () => {
          ...
          setMsg("Generated " + res.created + " slots ✓");
          router.refresh();
        }}
      >
        Generate slots
...
export function SlotRowActions({ slotId, maxBookings, isHoliday }: { slotId: string; maxBookings: number; isHoliday: boolean }) {
  ...
  return (
    <div className="inline-flex items-center gap-1.5">
      ...
      <button className="text-[11px] text-primary hover:underline" onClick={async () => { await updateSlot(slotId, { maxBookings: parseInt(m) || 1 }); router.refresh(); }}>Save</button>
      <button className="text-[11px] text-primary hover:underline" onClick={async () => { await updateSlot(slotId, { isHoliday: !isHoliday }); router.refresh(); }}>{isHoliday ? "Reopen" : "Holiday"}</button>
      <button className="text-[11px] text-destructive hover:underline" onClick={async () => { await deleteSlot(slotId); router.refresh(); }}>Delete</button>
    </div>
  );
}
