  ...
  return (
    <div className="space-y-4">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-xs text-muted-foreground">Lifetime <strong className="text-foreground tabular-nums">{formatRM(lifetime)}</strong></span>
      </div>
      <div className="space-y-3">
        {jobs.map((j) => (
          <div key={j.id} className="rounded-2xl border bg-card p-4">
            ...
            <div className="mt-1 font-semibold">{j.packageName ?? "General Service"} <span className="text-xs font-normal text-muted-foreground">· {j.jobNumber}</span></div>
