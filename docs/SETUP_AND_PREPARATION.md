...
| 2026-08-25 | A4 Demo 清理收尾（在 aeba0c5 基准上）：① demo actions 加生产守卫（setPersona/resetDemo 在 NODE_ENV=production 且无 NEXT_PUBLIC_DEMO_MODE 时拒绝——demoEnabled()，防生产误触 resetDemo 清空 34 张表）② rider/home 生产无顾客档案时显示登录引导（RiderSignInPrompt）替代 "Demo customer not found" 文案 ③ prisma/seed.ts 加 NODE_ENV=production 守卫（SEED_ALLOWED=1 覆盖），防生产误跑 seed 污染 | 生产纵深防御：demo persona 已由 middleware 拦截，本次封死 server action 与 seed 两条旁路；e2e（cookie 直设 persona + 非生产 NODE_ENV seed）不受影响；tsc/lint/unit 20/e2e smoke 57 全绿 |
| 2026-08-25 | 【彻底移除 demo 模式】用户要求不要任何 demo：删除 persona.ts/demo.ts/demo-user.ts/demo-customer.ts/reset.ts/actions/demo.ts/demo-bar.tsx 及 5 个 persona 审计脚本；middleware 只留 Supabase+legacy；session-user 纯 Supabase；rider 页改 getRiderCustomer；nav-registry 用 WorkshopPersona 类型；i18n 删 demo/persona 词条；.env 删 DEMO_MODE；e2e 改真实 Supabase 登录（helpers.setPersona→登录 DEMO_ACCOUNTS 账号 + global-setup 播种后 link-auth 回填 authId） | 生产无任何 demo：workshop/rider 登录后均无 demo bar（浏览器实测）；e2e 75 过/6 skip/0 败；部署 d-z-dhqv4laev；commit b7649ce（+233/-680）；persona 语义仅剩 nav-registry 角色导航分组（真实 Role→导航） |
| 2026-08-25 | 部署流程改革：① main force push 覆盖远程废弃历史（清除 12 个 QR/demo/Cloudflare commit），本地=远程=生产，分叉清零 ② 修复云端构建：src/providers/storage/supabase.ts 与 src/app/api/storage/[key]/route.ts 曾被 .gitignore 的 storage/ 规则误忽略（改 /storage/ 限定根目录）——GitHub 集成云端构建缺文件 module_not_found ③ 确立流程：本地开发→push feature 分支→Vercel Preview 验收→merge main→GitHub 集成自动生产部署；永不本地 vercel deploy --prod | 生产 = d-z-izf1dqez8（git source，GitHub 集成）；git HEAD 408d98c；踩坑：.gitignore 的 storage/ 误伤 src/providers/storage 与 src/app/api/storage；NEXT_PUBLIC_* 构建期内联（本地 vercel deploy 会带 .env 污染生产） |
| 2026-08-25 | 生产功能启用①：Vercel Cron 每日 09:00 服务提醒（vercel.json crons → /api/cron/reminders → sendDueReminders，CRON_SECRET Bearer 鉴权）；Preview env 配齐（DATABASE_URL=Supabase PG 等 10 项）；修复 provider 模块加载即抛错：SupabaseStorageProvider 懒加载 client、LocalStorageProvider 懒建目录（云函数 /var/task 只读，模块顶层 mkdir 会 ENOENT） | 生产实测：cron 无鉴权 401 / 正确鉴权 200 sent 14；生产 = d-z-37fyncooj（4666b3f）；首次走完完整流程：feature/cron-reminders → Preview（SSO 保护未浏览器验收，构建通过）→ merge main → GitHub 集成部署；踩坑：Preview 团队 SSO 保护、生产 env 需显式加 CRON_SECRET、provider 顶层实例化在 serverless 崩 |
| 2026-08-25 | 生产功能启用②：① 群发统计——calendar 按 Message.status 分组显示 sent/delivered/failed 徽章 + Broadcast 按钮展示已发数与送达详情 ② CDN asset URL——新增 src/lib/asset-url.ts（相对路径→NEXT_PUBLIC_BASE_URL 绝对 URL），rider news 海报轮播与产品图应用 ③ provider 换真预研——whatsapp-business.ts（Meta Graph API v19.0）+ openai.ts（gpt-4o-mini）骨架 + providers/index.ts env 驱动切换（WHATSAPP_API_TOKEN/OPENAI_API_KEY 存在即用真 provider，否则 mock，零代码改动） | 生产 = d-z-byjwn9pq0（36871db）；tsc 0/lint 0 err/unit 20/build 过；浏览器实测统计徽章（Broadcast (1) · 1 sent · 1 delivered）；流程完整：feature/prod-func-02 → 用户本地确认 → push → Preview Ready → merge main → GitHub 集成部署 |
| 2026-08-25 | Rider 端 4 个 bug fix（fix/bugfix-2026-08-25 → main 5a4ac1e）：① 摩托表单 SelectTrigger 加 w-full（下拉框与输入框等宽）② 表单布局重排（Year+Color 在 Brand 下两小列、Current Mileage 在 Model 下）③ Book 页 Date/Time 上下堆叠（space-y-4，各满宽 416px）④ News 海报不显示修复——assetUrl 对 /api/* 保持同源（不再拼 BASE_URL 跨端口）+ storage provider 改用 VERCEL=1 判断（next start 恒 production 曾误选 Supabase 404） | 生产 = d-z-jc7259wxh（5a4ac1e）；浏览器实测海报 1080×1920 正常渲染、表单对齐、Date/Time 堆叠；流程：分支→push→Preview Ready→用户确认 merge→GitHub 集成部署 |
