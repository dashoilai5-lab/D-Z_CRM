...
| 2026-08-25 | QR 系统上线（feature/qr-code → main 8f951a6，6 commit）：QR-001 摩托 QR（passport 展示，落地页车辆+车主+New job 预填）、QR-002 车主 QR（profile Member since 下，可点击放大 X 关闭）、QR-003 门店 QR（settings 展示+开关，rider 扫码确认进店绑定）；qrToken 不可枚举 token（Customer/Motorcycle/Organisation 唯一字段，迁移 20260825130000 回填，QR 编码 token 非 id）；Workshop 顶栏扫码器（zxing-js 相机扫码自动跳转）；落地页→业务动作（jobs/new?motorcycle 预填车主+摩托、customers/[id] 打开顾客）；schema.pg.prisma 同步 QR 字段 | 生产 = d-z-ia189py9v；tsc 0/build 过/e2e 75；浏览器实测 QR 三场景+放大+扫码按钮+预填全通过；依赖 react-qr-code 2.2.0 + @zxing/browser 0.2.1 + @zxing/library 0.23.0；本地 launchd 服务长期运行偶发 stream closed early（重启即恢复，生产 serverless 不受影响） |