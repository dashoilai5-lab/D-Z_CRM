...
| 2026-08-25 | 【生产修复】图片/海报生产 404：本地生成的海报/附件只在本地 ./storage，生产 Supabase bucket（dz-assets）为空 → /api/storage/* 404 → News 海报空白。已上传 14 个文件到生产 bucket（upsert）+ 修复 1 条旧 /storage/ URL + 验证生产上传链路（multipart→Supabase 公开 URL）；新增 scripts/sync-storage-to-supabase.ts 正式同步工具（幂等） | 生产 d-z-q6wal2oa8（44d0bbe）：poster/attach/product 全 200，News 海报 4 张正常显示；教训：本地生成的动态文件（海报/附件）必须同步生产 bucket，新工具可一键推送 |