...
| 2026-08-25 | 【生产修复】QR 迁移未应用到生产 PG：dashboard 报 Customer.qrToken does not exist（P2022）——迁移 20260825120000/130000 只应用了本地 dev.db，Vercel 构建不自动 migrate。用 node+pg 直接对生产 Supabase PG 执行：三表加 qrToken + Organisation 三开关 + md5(random()) 回填 107 顾客 + 唯一索引 | 生产恢复 d-z-ia189py9v：dashboard/settings/QR 落地页全 200，顾客 token 1b6db0e1、摩托 bd4fe7f6、门店 ef377d46；踩坑：Vercel 云端构建不跑 prisma migrate deploy，生产库 schema 变更需手动 SQL 应用（或构建脚本加 migrate）；SQLite 迁移方言（randomblob）不能直接 deploy PG，需 PG 兼容 SQL |