...
| 2026-08-25 | Rider 里程展示重构（feature/update-mileage → main 2156dac）：移除 home 页 Current Mileage 卡、progress bar、km-left/overdue；banner 与卡片只显示 Last service（里程+日期）与 Next service（里程+预计）；passport 顶部移除当前里程大数字；maintenance cycle 移除 wear 进度条与 used/interval，改为每项 LAST→NEXT 里程碑 | 生产 = d-z-5cprsqceb（2156dac）；浏览器实测 home/passport 均只显示服务节点；用户不再需要持续更新里程 |