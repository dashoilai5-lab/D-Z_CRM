...
| 2026-08-25 | 【生产修复】NEXT_PUBLIC_BASE_URL 错配：生产 env 指向 d-z-crm-dash-oil.vercel.app（Preview 子域，受 Vercel SSO 保护）→ Workshop QR 扫码要求登录 Vercel。已改为 https://d-z-crm.vercel.app（正式域名）并重新部署 | 三个 QR 落地页公开访问全 200，不再跳 SSO；影响面：所有用 BASE_URL 生成的链接（QR/提醒消息/海报分享）；QR 扫码目标确认：保持门店欢迎页 + Confirm 绑定设计 |