...
| 2026-08-26 | Rider 手机/邮箱二选一登录（feat/rider-phone-login）：登录页 password tab 输入框改「手机号或邮箱」；新增 src/lib/phone.ts 马来西亚号归一化（本地 013-125 2832 / 0131252832 / +60131252832 全兼容，E.164 去 60 前缀补回前导 0）；signInWithPassword 接受 identifier——手机号 → Customer.phone 归一化匹配 → authId → admin 查 auth email → email+password 登录（兼容现有账号）；phone-only 用户走 Supabase phone 登录（需 Dashboard 开 Phone provider）；i18n +1 词条 login.phone-or-email；tests/phone.test.ts 5 例 | 手机号或邮箱都能登录（password 模式）；OTP 模式仍仅 email（手机 OTP 需 SMS provider 后续）；workshop 登录不受影响（email 参数兼容）；unit 25/25 |