...
## 一句话状态
原型功能齐备且全绿（tsc0/unit20/e2e75+6skip），迭代增强完成：Loading 全链路、UI 去 AI 味与高级感、i18n toast、分页、里程审计流、Mechanic Board 技师下拉、preview 框架修复、海报自动轮播（按尺寸适配）、Workshop→Rider News 发布联动、产品图片（10 SKU）、Hot Picks、真机预览（LAN IP 192.168.100.240）。**已进入部署准备**：DEPLOYMENT_CHECKLIST.md。**A1 数据库迁移完成**（Supabase dukbfgqbrprivnzcsrlh：PG 基线 61 表/21 enum + 数据 5060 行全量，migrate deploy 不可行→diff 基线方案）。**A2 RLS 完成**（61 表 ENABLE RLS + 61 策略 docs/rls-policies.sql，生成器 scripts/gen-rls-policies.ts）。**A3 认证完成**（Supabase Auth：@supabase/ssr + client 三件套 + auth-supabase actions + middleware 三路径 + 登录页三 tab；User/Customer.authId 迁移 supabase_auth_link（注意：迁移目录名必须 YYYYMMDDHHMMSS 格式，13 位毫秒时间戳会排序靠前导致 no such table）；JWT claims 注入验证通过（owner@dz.my → Daniel Tan，claims 含 orgId/branchId/role/userId）；e2e 75+6skip 全绿）。下一步：A4 demo 清理 / 生产切 authenticated 角色连 PG。
