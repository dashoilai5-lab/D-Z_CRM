...
## 一句话状态
本地与生产 = main @ 294dd21（Mechanic App + Profile 工作统计）。今日已完成：…Rider Settings（个人资料编辑）→ Rider Settings 扩展（已上线）→ **Rider App 语言 bug 修复**（merge 33aad24：全站 i18n cbebcbd + Special Offer 卡片等 e4d2f6d）。流程约定：feature 分支 → 本地预览验证 → push → merge main → 自动部署（用户强调：先本地预览后才 push）。
...
3n. 【已完成】Mechanic Profile 工作统计（294dd21）：汇总（工单/客单价/评分）+ 12 个月月度趋势柱状（+8 业务月）
...
## git 状态
- 分支：main ｜ HEAD：294dd21（Profile 工作统计）｜ 未提交：仅 screenshots/ 截图
